...
export default class AccessControlService {
  public ac;
  public role;
  protected truePermission: Permission = { granted: true } as Permission;
  ...
  private getRoleAbilityLogic(ability: string): boolean | undefined {
    ...
    return (
      isEmpty(this.role.options) ||
      (this.role.options.abilities &&
        this.role.options.abilities.includes(ability))
    );
  }

  private setPermissionsForRecord(): void {
    if (this.getRoleAbilityLogic("can_create"))
      this.ac.grant(this.roleName).createAny("record");
    if (this.getRoleAbilityLogic("can_read"))
      this.ac.grant(this.roleName).readAny("record");
    if (this.getRoleAbilityLogic("can_update"))
      this.ac.grant(this.roleName).updateAny("record");
    if (this.getRoleAbilityLogic("can_delete"))
